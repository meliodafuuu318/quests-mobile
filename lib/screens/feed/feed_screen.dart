  ...
  Future<void> _loadFeed() async {
    ...
    try {
      ...
      if (res['error'] == false) {
        ...
        setState(() {
          _posts = (results is Map ? results['data'] : results) ?? [];
          _loading = false;
        });
      } else {
      ...
      setState(() { _error = 'Could not load feed: $e'; _loading = false; });
    }
  }
          ...
          if (_loading)
              ...
              child: Center(
                child: Column(mainAxisSize: MainAxisSize.min, children: [
                  ...
                  Padding(
                    ...
                    child: Text(_error!, style: AppTheme.label(color: AppTheme.textMuted), textAlign: TextAlign.center),
                  ),
            ...
            SliverList(
              delegate: SliverChildBuilderDelegate(
                (_, i) => Padding(
                  padding: const EdgeInsets.fromLTRB(20, 0, 20, 16),
                  child: _PostCard(post: _posts[i], onRefresh: _loadFeed),
                ),
...
class _PostCardState extends State<_PostCard> {
  ...
  Future<void> _toggleLike() async {
    ...
    try {
      await ApiService.react(widget.post['id'].toString());
    } catch (_) {
      setState(() { _liked = !_liked; _likeCount += _liked ? 1 : -1; });
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final postData = post['post'] is Map ? post['post'] : null;
    final title = postData?['title'] ?? post['title'] ?? '';
    final content = postData?['content'] ?? post['content'] ?? '';
    ...
    return GestureDetector(
      onTap: () => Navigator.push(
        context,
        MaterialPageRoute(builder: (_) => PostDetailScreen(postId: post['id'].toString())),
      ),
      child: Container(
            ...
            Padding(
              ...
              child: Row(children: [
                ...
                Container(
                  ...
                  child: Text(visibility.toUpperCase(), style: AppTheme.label(color: AppTheme.textMuted, size: 10)),
                ),
              ]),
            ),
            // Body
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const SizedBox(height: 12),
        ShimmerBox(width: double.infinity, height: 55),
      ]),
    );
  }
}